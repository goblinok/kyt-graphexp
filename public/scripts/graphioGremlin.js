...
var graphioGremlin = (function(){
	...
	function get_graph_info(){
		...
		var message = ""
          ...
          $('#outputArea2').html(">> <p> node_label_query: " + node_label_query + "</p>" +
																		"<p> edge_label_query: " + edge_label_query + "</p>" +
																		"<p> node_prop_query: " + node_prop_query + "</p>" +
																		"<p> edge_prop_query: " + edge_prop_query + "</p>" );
          ...
          $('#outputArea2').html(">> <p> Node query: " + nodeQuery + "</p>" +
              "<p> Edge query: " + edgeQuery + "</p>");
    ...
    if (sql_field !== "") {
      //edgeQuery = sql_field;
      //https://www.slideshare.net/slidarko/the-gremlin-traversal-language
      gremlin_query_nodes = sql_field;
    }
    let gremlin_query_nodes_org = gremlin_query_nodes.replace("nodes = ", "").replace(".toList();", "")
      ...
      //TEST
			var edgeQuery = gremlin_query_nodes_org + ".aggregate('node').outE().as('edge').inV().where(within('node')).select('edge').toList();"
      ...
      $('#outputArea2').html(">>" + gremlin_query)
  ...
  function send_to_server(gremlin_query,query_type,active_node,message, callback){
    ...
    if (COMMUNICATION_PROTOCOL == 'REST'){
      ...
      // Neptune이면 VPC안의 인스턴스(Proxy)에서 실행되도록 한다.
      if (server_address == '172.30.0.224') {
        server_url = "http://poc-bat-alb-161916887.us-east-1.elb.amazonaws.com:2081/graph";
      }
      else {
      ...
      // Neptune이면 VPC안의 인스턴스(Proxy)에서 실행되도록 한다.
      if (server_address == '172.30.0.224') {
        server_url = "ws://poc-bat-alb-161916887.us-east-1.elb.amazonaws.com:2081/graph";
      }
      else {
        server_url = "ws://"+server_address+":"+server_port+"/gremlin"
      }
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function handle_server_answer(data,query_type,active_node,message){
      ...
      $('#outputArea3').html(">>> <p> data : " + JSON.stringify(data)+ "</p>" +
					                       "<p> graph : " + JSON.stringify(graph) + "</p>");
